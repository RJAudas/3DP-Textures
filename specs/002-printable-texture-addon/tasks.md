...
- [X] T001 Create the package/dir layout from plan.md § Project Structure: `3dp_textures/`,
  `3dp_textures/engine/`, and `tests/` (empty `__init__.py` in `3dp_textures/` and
  `3dp_textures/engine/` as needed).
- [X] T002 [P] Create `3dp_textures/blender_manifest.toml` (`schema_version="1.0.0"`,
...
- [X] T003 Create `3dp_textures/__init__.py` with skeleton `register()`/`unregister()` that import
  and (un)register submodules in order, with reverse-order teardown (symmetric, idempotent).
- [X] T004 [P] Create the headless test harness `tests/run_headless.py` (discovers/runs test
...
- [X] T005 [P] Implement `SurfaceSettings` `PropertyGroup` in `3dp_textures/properties.py` with all
...
- [X] T006 [P] Implement the `Preset` registry in `3dp_textures/presets.py`: `wood` fully
  configured (`TEX_WOOD`, strength 0.4 mm, scale 1.0, LOCAL, NORMAL, mid 0.5) plus `brick`/`rock`
  named stub entries.
- [X] T007 [P] Implement auto subdivision-level calculation in `3dp_textures/subdivision.py`
  (world-space bbox → Simple-subsurf `levels` targeting ~0.2–0.5 mm edge length).
- [X] T008 Implement the strategy interface + registry in `3dp_textures/engine/__init__.py`
  (`build(obj, settings)`, `update(obj, settings)`, `teardown(obj)` contract; registry keyed by
  `strategy`).
- [X] T009 Implement the panel skeleton `VIEW3D_PT_texture_to_geometry` in `3dp_textures/ui.py`
  (N-panel tab "Texture → Geometry", disabled-with-hint when no active mesh object).
- [X] T010 Wire `register()`/`unregister()` in `3dp_textures/__init__.py` to (un)register
  `properties`, `presets`, `engine`, and `ui` (and the `Object.tdp_surface` pointer) symmetrically.
- [X] T011 [P] Implement `tests/test_register.py`: enable → disable → re-enable leaves no
...
- [X] T012 [P] [US1] `tests/test_displace_procedural.py`: after `tdp.apply_texture` (WOOD) on a
  cylinder, the **evaluated** mesh bounds/positions changed beyond tolerance; no NaN coords; no
  zero-area faces.
- [X] T013 [P] [US1] `tests/test_export_stl.py`: `tdp.export_stl` writes a file whose re-read
  triangle geometry differs from the undisplaced base mesh (relief present → SC-002).

### Implementation for User Story 1

- [X] T014 [US1] Implement the primary A1 strategy in `3dp_textures/engine/displace_procedural.py`:
...
- [X] T015 [US1] Implement the `tdp.apply_texture` operator in `3dp_textures/operators.py`
...
- [X] T016 [US1] Implement the `tdp.export_stl` operator in `3dp_textures/operators.py`: select
...
- [X] T017 [US1] Wire the MVP UI in `3dp_textures/ui.py`: Preset selector, Strength (mm), Scale,
  and **Apply Texture** + **Export STL…** buttons with tooltips.
- [X] T018 [US1] Register the US1 operators in `3dp_textures/__init__.py` register/unregister lists.
...
- [X] T019 [P] [US2] `tests/test_nondestructive.py`: after `tdp.apply_texture` then `tdp.clear`
...
- [X] T020 [US2] Add live-update behavior to `SurfaceSettings` in `3dp_textures/properties.py`:
...
- [X] T021 [US2] Implement the `tdp.bake` ("Apply now") operator in `3dp_textures/operators.py`:
  apply `3DP Subdivision` then `3DP Displace`, remove the add-on modifiers, report new vert/face
  counts (state → baked).
- [X] T022 [US2] Implement the `tdp.clear` operator in `3dp_textures/operators.py`: remove `3DP `
  modifiers + owned texture, restoring the base mesh (state → none).
- [X] T023 [P] [US2] Implement printability warnings in `3dp_textures/validation.py` (strength vs.
  smallest world-space dimension; detail finer than typical nozzle) surfaced via
  `self.report({'WARNING'})` (FR-013).
- [X] T024 [US2] Extend `3dp_textures/ui.py`: an Advanced sub-section (Coordinate mode, Direction,
...
- [X] T025 [US2] Register the US2 operators in `3dp_textures/__init__.py`.
...
- [X] T026 [P] [US3] `tests/test_displace_image.py`: `strategy='IMAGE'` with a generated grayscale
  height-map displaces the evaluated mesh; missing image → `{'CANCELLED'}` + ERROR report.
- [X] T027 [P] [US3] `tests/test_shapes.py`: the procedural Wood workflow produces relief on both a
  flat plane and a sphere (Direction=Normal wraps the curved surface) — FR-010.

### Implementation for User Story 3

- [X] T028 [US3] Implement the A2 fallback strategy in `3dp_textures/engine/displace_image.py`:
...
- [X] T029 [US3] Extend `3dp_textures/ui.py`: Strategy switch (Procedural/Image), image/height-map
  picker (shown for Image), and the UV-seam/stretch warning on curved shapes.
- [X] T030 [P] [US3] Complete the `brick` and `rock` preset configurations in
  `3dp_textures/presets.py` (rock → `TEX_NOISE`/`TEX_VORONOI`; brick → procedural or image source).
- [X] T031 [US3] Ensure the engine registry in `3dp_textures/engine/__init__.py` resolves the
...
- [X] T032 [P] Tooltip + units pass across `3dp_textures/ui.py` and `3dp_textures/properties.py`
  (every property/operator has a tooltip; length fields use scene units — Platform Standards).
- [X] T033 [P] Write `README.md` (install from disk, the three quickstart scenarios, 4.2 LTS
  requirement, link to research deliverables).
- [X] T034 Verify reload discipline (install → enable → disable → re-enable, no console errors) and
  run the full headless suite `blender --background --python tests/run_headless.py` — all pass
  (Definition of Done).
- [X] T035 Build/validate the extension package and confirm `blender_manifest.toml`
  `[build].paths_exclude_pattern` excludes `tests/` and caches from the shipped artifact.
- [X] T036 Execute the `specs/002-printable-texture-addon/quickstart.md` validation scenarios
